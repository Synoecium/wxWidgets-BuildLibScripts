...
rm -f "CMakeCache.txt"
cmake -G "Unix Makefiles" -DwxBUILD_SHARED=OFF -DCMAKE_BUILD_TYPE=Debug
...
cmake -G "Unix Makefiles" -DwxBUILD_SHARED=OFF -DwxBUILD_DEMOS=ON -DCMAKE_BUILD_TYPE=Release
cmake --build .
if [ $? -ne 0 ] ; then 
	exit $? 
fi
